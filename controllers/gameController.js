...
export const createUserScore = async (req, res) => {
  try {
    ...
    const shard = await getShardNumber();
    const log = await saveShard(shard, "GAME_DB", "score", GAME_SQL_QUERIES.CREATE_USER_SCORE, playerId, [playerId, score]);
    res.status(201).json(log);
  } catch (error) {
    console.error(error);
...
export const createUserRating = async (req, res) => {
  try {
    ...
    const shard = await getShardNumber();
    const log = await saveShard(shard, "GAME_DB", "rating", GAME_SQL_QUERIES.CREATE_USER_RATING, playerId, [playerId, characterId, win, lose]);
    res.status(201).json(log);
  } catch (error) {
    console.error(error);
